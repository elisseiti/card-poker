...
Once you are finished with the task, please click the `Submit Solution` link on <a href="https://app.codescreen.com/candidate/6380ef02a8e06043387a279a" target="_blank">this screen</a>.

## Project Explanation
### Only Texas Holdem Poker ranking is implemented

#### A PokerCard is represented as a case class which contains two enum parameters: `CardRank` and `CardSuit`.
#### `CardRank` has a compare method which we will use to sort a List[PokerCard]

#### The Extractor class gets a list of Poker Cards with five elements.
#### The list of PokerCards is matched to one of the specific poker hand value.
#### The class PokerHand contains all the sealed abstract classes and extended classes that match the poker hand.
#### The class PlayerHandOrdering contains the logic how to compare PlayerHand. We follow the same principles as a poker game.
#### In the class we compare PokerHands of the same category. The method indexOf helps us to set a value to each category and then we compare two PokerHands of not the same type by just checking the difference between their index.
#### We compare Pokerhands of the same type by creating a respective method for each of the classes.
#### `Straight` and `StraightFlush` are compared only by getting the highest card rank.
#### The highest card just compares two lists recursively value by value.
#### The Remaining methods are sorted by cards of same rank and rank value. They are sorted in descending order. Then we compare the list elements by traversing both lists at the same time.
#### Texas HoldemHelper class contains the logic that transforms the string input to a list of cards and how to get the highest card from the combination of the board cards and the player cards.
### Limitations
#### 1. Contains duplicated code. Could be solved by using sum types and union types, specifically at `PokerHandOrdering`.
#### 2. Could not make it work with the implicit keyword. Would have been very handy with the `Ordering` classes.
